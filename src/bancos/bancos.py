...
class Banco():
    def __init__(self, entry=None, statusbar=None, panel=None):
        self.widgets = gtk.glade.XML('../bancos/bancos.glade')
        ...
        self.widgets.get_widget('statusbar1').push(1,'Busque un banco')
        self.statusbar.push(1,'Busque un banco')
        self.insertPanel('INFO','Abierta tabla de bancos')
        ...
        self.cursor.execute('select id, nombre from bancos order by id')
    ...
    def search(self, data=None):
        ...
        if (searchById.get_active() == True):
            ...
            lista = list()
            for tupla in tuplas:
                if (string.lower(tupla[1]).find(string.lower((str(data.get_text())))) == 0):
                    lista.append(tupla)
            self.fillData(lista)
    ...
    def clickInsertProvincia(self, data=None):
        ...
        self.widgets.get_widget('statusbar1').push(4,'Insertando banco...')
        self.statusbar.push(4,'Insertando banco...')
        self.cursor.execute('select max(id)+1 from bancos')
    ...
    def clickDeleteProvincia(self, data=None):
        self.widgets.get_widget('statusbar1').push(5,'Eliminando banco...')
        self.statusbar.push(5,'Eliminando banco...')
        value = self.getSelection('borrar')

        if value:
            self.cursor.execute("select * from bancos where id = %s", value)
            tupla = self.cursor.fetchone()
            ident = str(tupla[0])
            nombre = str(tupla[1])

            dialog = self.widgets.get_widget('messagedialog2')
            dialog.format_secondary_text('Desea eliminar ('+ident+', '+unicode(nombre,'iso8859-15')+')')
            dialog.show()

    def clickModifyProvincia(self, data=None):
        self.widgets.get_widget('statusbar1').push(6,'Modificando banco...')
        self.statusbar.push(6,'Modificando banco...')
    ...
    def dialogModifyProvincia(self, data=None):
        ...
        dialog.format_secondary_text('¿Son correctos los cambios? ('+ident+', '+unicode(nombre,'utf 8')+')')
        dialog.show()
    ...
    def acceptModifyProvincia(self, data=None):
        ...
        try:
            self.cursor.execute("update bancos set nombre = %s where id = %s", (nombre, ident))
            self.cursor.execute("select * from bancos order by id")
            tuplas = self.cursor.fetchall()
            tuplas = self.normalizar(tuplas)
            self.fillData(tuplas)   
            self.widgets.get_widget('window3').hide()
            self.insertPanel('OK','Modificación de banco realizada correctamente')
    ...
    def acceptDeleteProvincia(self, data=None):
        ...
        if value:
            self.cursor.execute("delete from bancos where id = %s", value)
            self.cursor.execute("select * from bancos order by id")
            tuplas = self.cursor.fetchall()
            tuplas = self.normalizar(tuplas)
            self.fillData(tuplas) 
            self.insertPanel('OK','Banco eliminada correctamente')
    ...
    def acceptNewProvincia(self, data=None):
        ...
        try:
            ...
            if(nombre == ''):
                raise ValueError
            aux = [long(identificador), unicode(nombre,'utf 8')]
            try:
                self.cursor.execute('insert into bancos(id, nombre) values (%s, %s)', aux)
                self.cursor.execute('select * from bancos order by id')
                tuplas = self.cursor.fetchall()
                tuplas = self.normalizar(tuplas)
                self.fillData(tuplas) 
                self.hideWindow2()
                self.insertPanel('OK','Nueva banco creada correctamente')
            except MySQLdb.Error, e:
                self.error(e)
        except ValueError:
    ...
    def getSelection(self, msg):
        ...
        try:
            ...
            self.error('Seleccione el banco que desea '+msg)
            return False
    ...
    def doubleClick(self, widget, ident=None, data=None):
        self.cursor.execute('select nombre from bancos where id = %s', self.getSelection('insertar'))
        tupla = self.cursor.fetchone()
        if tupla:
            value = unicode(str(tupla[0]), 'iso8859-15')
